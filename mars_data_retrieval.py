...
#DATA RETRIEVAL & PARSING. JSON STORAGE
...
print "Request Successful: %s" % (successful_resp)




save_data_file_name = "./json_storage/insight_data_storage.json"
...
with open(save_data_file_path) as json_file:
...
def get_table_id(ds):
	return original_tables[ds]

def get_table(sol, ds):
	set_id = get_table_id(ds)
	return data[sol][set_id]

original_tables = {}
table_ids = ["AT", "HWS", "WD", "PRE"]
...
#Enumerate over each string table ID, set it equal to an enum
for i,ele in enumerate(table_ids):
	original_tables[Datasets(i)] = ele
...
sol_ids = range(0,len(data["sol_keys"]))


def get_day_of_year(sol_id):
	return days_into_year[sol_id]
...
def organise_std_data(ds_id, sol_ids):
	...
	for sol in sol_ids:
		print sol
		day_of_year = get_day_of_year(sol)
